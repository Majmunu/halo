  ...
  <div class="root col-xl-9 clearfix">
    <div class="bread">
      <Breadcrumb>
        ...
        <BreadcrumbItem to="/jiaocheng">教程</BreadcrumbItem>
        <BreadcrumbItem>教程详情</BreadcrumbItem>
      </Breadcrumb>
    </div>
 ...
 data () {
  ...
  methods: {
    ...
    send() {
      ...
      this.request.get("/coursecomment/tree/"+this.id).then(res => {
    ...
    saveComment() {
      ...
      this.request.post("/coursecomment/", this.commentForm).then(res => {
        if (res.code === '200') {
    ...
    delComment(id) {
      this.request.delete("/coursecomment/" + id).then(res => {
        if (res.code === '200') {
          this.$message.success("删除成功")
          this.loadComment()
        } else {
